...
const DialogRegOferta = ({
  ...
  useEffect(() => {
    setCarreraOferta({
      id: datosEditarCarrera?.id_carrera,
      nombre: datosEditarCarrera?.nb_carrera
    })
    setCodOferta(datosEditarCarrera?.co_oferta)
    setCantidadCupos(datosEditarCarrera?.nu_cupos)
    setSedeOferta({
      id: datosEditarCarrera?.id_sede.toString(),
      nombre: datosEditarCarrera?.nb_sede
    })
    setPeriodoOfer(datosEditarCarrera?.id_periodo)
  }, [datosEditarCarrera, activeDialogEditarCarrera, sedes])

  const editarOferta = (variables) => {
    return request(
      process.env.NEXT_PUBLIC_URL_BACKEND,
      GQLregOfertaAcademica.EDIT_OFERTA_ACADEMICA,
      variables
    )
  }
    ...
    if (validacionPersonal) {
      ...
      if (
        ...
        const InputOferta = {
          ...
          periodoOfer: parseInt(periodoOfer),
          objectOferta: detallesMallas?.obtenerDetalleMalla.response
        }

        editarOferta({
          idOferta: datosEditarCarrera?.id_oferta,
          InputOferta
        }).then(({ editarOferta: { status, message, type } }) => {
          toast.current.show({
            severity: type,
            summary: '¡ Atención !',
            detail: message
        ...
        <div className="grid grid-cols-5 gap-4 m-2">
          ...
          <span className="p-float-label field">
            <Dropdown
              ...
              value={carreraOferta}
              options={mallas?.obtenerTodasMallas.response.materias}
