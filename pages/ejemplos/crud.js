...
export default function Crud() {
  ...
  const { data: dataGrupos } = useSWR(
    token ? [GQLPlantilla.GET_GRUPOS, {}, token] : null
  ) */
  const { data: dataGrupos } = useSWR([GQLPlantilla.GET_GRUPOS, {}])

  /* const {
    data: dataPersonas,
    error,
    mutate
  } = useSWR(token ? [GQLPlantilla.GET_PERSONAS, {}, token] : null) */

  const {
    data: dataPersonas,
    error,
    mutate
  } = useSWR([GQLPlantilla.GET_PERSONAS, {}])

  const borrarPersona = (variables) => {
    return request(
      process.env.NEXT_PUBLIC_URL_BACKEND,
      GQLPlantilla.DELETE_PERSONA,
      variables,
  ...
  const header = (
    <div className="table-header text-left">
      <h5 className="mx-0 my-1 text-center text-3xl font-bold text-indigo-900">
        Gestionar Personas
      </h5>
  ...
  const deletePersona = () => {
    setSubmitted(true)
    borrarPersona({ id: persona.id_persona })
      .then(() => {
        mutate()
        setSubmitted(false)
        setDeletePersonaDialog(false)
  ...
  const deleteSelectedPersonas = () => {
    setSubmitted(true)
    borrarPersonas({ ids: selectedPersonas.map((p) => p.id_persona) })
      .then(() => {
        mutate()
        setSubmitted(false)
        setDeletePersonasDialog(false)
        ...
        toast.current.show({
          severity: 'success',
          summary: '',
          detail: 'Personas eliminadas exitosamente.',
          life: 3000
        })
      })
      .catch(({ response: { errors } }) => {
  ...
  const bodyTelefono = (row) => {
    return (
      <span>
        ({row.telefono.substring(0, 4)}) {row.telefono.substring(4, 7)}-
        {row.telefono.substring(7)}
      </span>
    )
  }
    ...
    return (
      <Fragment>
        <Button
          ...
          onClick={() => editPersona(rowData)}
        />
        <Button
          icon="pi pi-trash"
          className="p-button-rounded p-button-warning"
          onClick={() => confirmDeletePersona(rowData)}
        />
      </Fragment>
    )
  }
    ...
    <AppLayout>
      ...
      >
        <div className="confirmation-content">
          ...
          {persona && (
            <span>
              ¿Desea eliminar a <b>{persona.nb_persona}</b>?
            </span>
          )}
        </div>
      </Dialog>
      ...
      >
        <div className="confirmation-content">
          ...
          />
          {persona && (
            <span>¿Desea eliminar a todas las personas seleccionadas?</span>
          )}
        </div>
      </Dialog>
      <Dialog
