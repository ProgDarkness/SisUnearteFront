...
export default function Guardar() {
  ...
  const { data } = useSWR([GQLPlantilla.GET_GRUPOS, {}])

  const guardarPersona = (variables) => {
    return request(
      process.env.NEXT_PUBLIC_URL_BACKEND,
      GQLPlantilla.SAVE_PERSONA,
      variables
  ...
  const guardar = (e) => {
    e.preventDefault()
    const { nb_persona, telefono, id_grupo } = persona
    if (!nb_persona || !telefono || !id_grupo) {
      toast.current.show({
        severity: 'warn',
        summary: '',
        ...
        <div className="grid gap-y-4">
          <div className="p-inputgroup">
            ...
            <div className="p-float-label">
              <InputText
                id="nombre"
                autoComplete="off"
                value={persona.nb_persona}
                onChange={({ target: { value } }) =>
                  setPersona((ps) => ({ ...ps, nb_persona: value }))
                }
              />
          ...
          <div className="p-inputgroup">
            ...
            <div className="p-float-label">
              <InputMask
                ...
                onChange={({ target: { value } }) =>
                  setPersona((ps) => ({ ...ps, telefono: value }))
                }
              />
          ...
          <div className="p-inputgroup">
            ...
            <span className="p-float-label">
              <Dropdown
                id="grupo"
                value={persona.id_grupo}
                options={data?.grupos}
                optionLabel="grupo"
                optionValue="id_grupo"
                onChange={({ value }) =>
                  setPersona((ps) => ({ ...ps, id_grupo: value }))
                }
              />
